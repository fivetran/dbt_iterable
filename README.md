...
| [iterable__journeys](https://fivetran.github.io/dbt_iterable/#!/model/model.iterable.iterable__journeys) | Tracks journey performance with engagement metrics, unique user counts, and can derive engagement rates rolled up from the campaigns belonging to each journey, so you can measure which automated sequences actually work. Journeys that send no messages, such as audience splits, are still included. <br></br>**Example Analytics Questions:**<ul><li>Which automated journeys are driving the most engagement?</li><li>How does our welcome series compare to our cart abandonment sequence?</li><li>Which journeys are enabled but no longer reaching anyone?</li></ul>|
...
vars:
    ...
    iterable__using_journey: false                 # default is true
```
...
This package includes fields we judged were standard across Iterable users. However, the Fivetran connector allows for additional columns to be brought through in the `event_extension`, `journey`, and `user_history` objects. Therefore, if you wish to bring them through, leverage our passthrough column variables. For `event_extension` columns, ensure that `iterable__using_event_extension` is set to True, which is the default. The same applies to `iterable__using_journey` for `journey` columns.

You will see these additional columns populate in the end `iterable__list_user_history`, `iterable__events`, `iterable__journeys`, and `iterable__users` models.

The `journey` object includes a set of `draft_*` fields whose exact names vary by connection, so they are not included by default. Use `iterable_journey_pass_through_columns` to bring through the specific draft fields you need.
